...
namespace openchat {
    class neuralNetwork {
        ...
        public:
            void init() {
                this->gain = std::vector<float>(dimensions.size() - 1);
                this->bias = std::vector<float>(dimensions.size() - 1);
            ...
            void readFromFile(std::pair<std::filesystem::path, std::vector<std::filesystem::path>> input) {
                ...
                if (inFile.is_open()) {
                  inFile.read(reinterpret_cast<char *>(&this->gain),
                              sizeof(float) * (this->dimensions.size() - 1));
                  inFile.read(reinterpret_cast<char *>(&this->bias),
                              sizeof(float) * (this->dimensions.size() - 1));
                }
            ...
            void saveToFile(std::pair<std::filesystem::path, std::vector<std::filesystem::path>> output) {
                ...
                if (outFile.is_open()) {
                    outFile.write(reinterpret_cast<char *>(&this->gain), sizeof(float) * (this->dimensions.size() - 1));
                    outFile.write(reinterpret_cast<char *>(&this->bias), sizeof(float) * (this->dimensions.size() - 1));
                }
